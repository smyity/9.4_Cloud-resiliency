...
resource "yandex_compute_instance" "bastion" {
  ...
  scheduling_policy { preemptible = true } # останавливаемая машина

  network_interface {
    subnet_id          = yandex_vpc_subnet.block_1.id #зона ВМ должна совпадать с зоной subnet!!!
    nat                = true # предоставление внешнего ip адреса
...
resource "yandex_compute_instance" "ubuntu_workstation" {
  ...
  network_interface {
    ...
    nat                = false
    security_group_ids = [ yandex_vpc_security_group.LAN.id, yandex_vpc_security_group.locker.id ]
  }
}
...
${vm.name} ansible_host=${vm.network_interface.0.ip_address} ansible_port=${var.ssh_port} ansible_user=${var.vm_user} ansible_ssh_private_key_file=~/.ssh/ssh-key-1759501063847
%{ endfor ~}

[target_group:vars]
ansible_ssh_common_args='-o ProxyJump=${var.vm_user}@${yandex_compute_instance.bastion.network_interface.0.nat_ip_address}:${var.ssh_port}'
XYZ
}
...
[bastion]
ssh ${var.vm_user}@${yandex_compute_instance.bastion.network_interface.0.nat_ip_address} -p ${var.ssh_port}
%{ for vm in yandex_compute_instance.ubuntu_workstation }
[${vm.name}]
ssh -J ${var.vm_user}@${yandex_compute_instance.bastion.network_interface.0.nat_ip_address}:${var.ssh_port} ${var.vm_user}@${vm.network_interface.0.ip_address} -p ${var.ssh_port}
%{ endfor ~}
EOF
}
